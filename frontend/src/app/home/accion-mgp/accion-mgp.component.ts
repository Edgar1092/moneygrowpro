...
@Component({
  selector: 'app-accion-mgp',
  templateUrl: './accion-mgp.component.html',
  styleUrls: ['./accion-mgp.component.scss']
})
export class AccionMGPComponent implements OnInit {

  blogs2$: Observable<any[]>;
  total = 0;
  p=1;
  itemsPerPage = 5;
  formBlog: FormGroup;
  ...
  constructor(private AccionService: AccionService, private toast: ToastrService, 
    ...
    this.blogs2$ = this.AccionService.blogs2$;
  ...
  loadInitialData(params){
    this.AccionService.getAccionmgp(params);
  ...
  //   this.loadInitialData(filterParams);
  // }
