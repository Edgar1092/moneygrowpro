...
@Component({
  selector: 'app-accionmgp-add',
  templateUrl: './accionmgp-add.component.html',
  styleUrls: ['./accionmgp-add.component.scss']
})
export class AccionmgpAddComponent implements OnInit {
 ...
 toggle() {
  ...
  add() {
    ...
    if (this.formBlog.valid) {
      ...
      this.AccionService.addMGP(this.formBlog.value).subscribe(response => {
        if (response) {
          
          this.toast.success(response['message']);
          this.router.navigate(['/home/accionMGP']);
        } else {
